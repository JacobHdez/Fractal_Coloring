...
#=
Choise of function
    A the C-shore → f(z) = √((1+i)z - 1)
=#
Cshore(z) = sqrt((1 + im) * z - 1)

println( Cshore(Sym("z")) )
println( Cshore(Sym("x") + Sym("y")*im) )
...
Cshore_img = zeros(RGB{Float64}, (pixelsY, pixelsX));
...
for x = Xeval
    ...
    for y = Yeval
        ...
        elseif k < 0 || k > pi/2
            ...
            Cshore_img[j, i] = RGB{Float64}(Red_int(k)/255, Green_int(k)/255, Blue_int(k)/255)
        end
...
save(string("results\\A_the_C-shore_", string(max_iteration), "_", string(pixelsX), "x", string(pixelsY), ".png"), Cshore_img)
plot(Cshore_img)
