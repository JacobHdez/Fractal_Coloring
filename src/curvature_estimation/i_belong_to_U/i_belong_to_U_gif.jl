...
#=
Choise of function
    i belong to U → f(z) = ((1-i)z^6+(7+i)z)/(2z^5+6)
=#
Corona(z) = ((1 - im)*z^6 + (7 + im)*z) / (2*z^5 + 6)

println( Corona(Sym("z")) )
println( Corona(Sym("x") + Sym("y")*im) )
...
pixelsX = 48 # 240 480 960 1920 3840 7680 15360
pixelsY = 27 # 135 270 540 1080 2160 4320 8640

Xeval = X_intv[1] : (X_intv[2] - X_intv[1]) / (pixelsX - 1) : X_intv[2]
Yeval = Y_intv[1] : (Y_intv[2] - Y_intv[1]) / (pixelsY - 1) : Y_intv[2]

anim = @animate for max_iteration = 3:23
    println("Iteracion: ", max_iteration)

    Corona_img = zeros(RGB{Float64}, (pixelsY, pixelsX));
    ...
    for x = Xeval
        ...
        for y = Yeval
            ...
            elseif k < 0 || k > pi/2
                ...
                Corona_img[j, i] = RGB{Float64}(Red_int(k)/255, Green_int(k)/255, Blue_int(k)/255)
            end
...
gif(anim, string("results\\i_belong_to_U_", string(pixelsX), "x", string(pixelsY), "_fps5.gif"), fps = 5)
