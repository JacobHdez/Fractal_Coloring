    ...
    for x = Xeval
        ...
        for y = Yeval
            ...
            elseif k < 0 || k > pi/2
                ...
                Mandelbrot_img[j, i] = RGB{Float64}(Red_int(k)/255, Green_int(k)/255, Blue_int(k)/255)
            end
...
gif(anim, string("results\\MandelbrotSet_", string(pixelsX), "x", string(pixelsY), "_fps5.gif"), fps = 5)
